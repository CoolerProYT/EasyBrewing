...
public class NeoForgeCommonConfig {
    public static final NeoForgeCommonConfig CONFIG;
    ...
    static {
        Pair<NeoForgeCommonConfig, ModConfigSpec> pair = new ModConfigSpec.Builder().configure(NeoForgeCommonConfig::new);
    ...
    /**
     * Syncs NeoForge config values to the common config static fields.
     * Should be called after config is loaded/reloaded.
     */
    public static void syncToCommon() {
        CommonConfig.potionStackSize = CONFIG.potionStackSize.get();
        CommonConfig.maxSpeedUpgrade = CONFIG.maxSpeedUpgrade.get();
        CommonConfig.allowAmountUpgrade = CONFIG.allowAmountUpgrade.get();
        CommonConfig.maxAmountUpgrade = CONFIG.maxAmountUpgrade.get();
        CommonConfig.potionCount = CONFIG.potionCount();
        CommonConfig.processingTime = CONFIG.processingTime.getAsInt();
        CommonConfig.cobblemonPotionCount = CONFIG.cobblemonPotionCount.get();
    }

    public int potionCount(){
        if (potionCount.get() > potionStackSize.get()) {
            Constants.LOGGER.warn("potionCount ({}) is greater than potionStackSize ({}). Clamping.", potionCount.get(), potionStackSize.get());
        }
